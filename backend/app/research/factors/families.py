...
from app.research.factors.transforms import zscore
...
def _latest_pit_per_symbol(
    ...
    Caller is responsible for filtering ``available_at <= as_of`` *before*
    invoking the family builder. We do not re-filter here so the family fn
    stays composable with whatever pre-filter strategy the runner uses.
    """
...
def _latest_close_per_symbol(kline: pl.DataFrame) -> pl.DataFrame:
    ...
    if kline.is_empty():
        return pl.DataFrame(schema={"symbol": pl.Utf8, "event_time": kline.schema.get("event_time", pl.Datetime), "close": pl.Float64})
...
def value_factor_pb(
    ...
    Args:
        fundamentals: PIT fundamentals frame. Must include columns
            ``symbol``, ``event_time``, ``book_value``. Consumer must
            pre-filter to ``available_at <= as_of`` so the latest visible
            book value is selected per symbol.
        kline: PIT OHLCV frame (same shape used by ``pipeline/runner.py``).
            Latest visible close per symbol is used as the price input.

    Returns:
        Long-form factor frame matching the runner's ``factor`` long format.
        Empty frame if either input is empty or columns are missing in a way
        that makes the join impossible.

    NOTE: A fundamentals lake producer is **not yet built** in this repo. This
    function ships the transform that *would* compute the factor once such a
    producer exists; the consumer (the runner, eventually) must supply the
    frame. See :data:`VALUE_PB_SPEC` for required columns.
    """

    required = {"symbol", "book_value"}
    if fundamentals.is_empty() or not required.issubset(set(fundamentals.columns)):
        return _to_long(pl.DataFrame(schema={"symbol": pl.Utf8, "timestamp": pl.Datetime(time_zone="UTC"), "value": pl.Float64}), VALUE_PB_SPEC.name)
    ...
    if joined.is_empty():
        return _to_long(joined.rename({"event_time": "timestamp"}).with_columns(pl.lit(None).alias("value")), VALUE_PB_SPEC.name)
...
def quality_factor_roe(fundamentals: pl.DataFrame) -> pl.DataFrame:
    ...
    Args:
        fundamentals: PIT fundamentals frame. Must include
            ``symbol``, ``event_time``, ``net_income``, ``equity``.

    Returns:
        Long-form factor frame matching the runner's ``factor`` long format.
        Rows with ``equity <= 0`` are dropped (undefined ROE).

    NOTE: Same lake caveat as :func:`value_factor_pb` -- the producer is not
    yet built.
    """

    required = {"symbol", "net_income", "equity"}
    if fundamentals.is_empty() or not required.issubset(set(fundamentals.columns)):
        return _to_long(pl.DataFrame(schema={"symbol": pl.Utf8, "timestamp": pl.Datetime(time_zone="UTC"), "value": pl.Float64}), QUALITY_ROE_SPEC.name)

    time_col = "event_time" if "event_time" in fundamentals.columns else None
    latest = _latest_pit_per_symbol(fundamentals)
    latest = latest.filter(pl.col("equity") > 0)
    if latest.is_empty():
        return _to_long(pl.DataFrame(schema={"symbol": pl.Utf8, "timestamp": pl.Datetime(time_zone="UTC"), "value": pl.Float64}), QUALITY_ROE_SPEC.name)
...
def momentum_resid_volatility(
    ...
    Runs on OHLCV-only data already produced by the existing runner fixture --
    no fundamentals required, so this can drop into the lazy factor pass today.

    Args:
        kline: PIT OHLCV frame as produced by ``pipeline/runner.py``. Uses
            ``adjusted_close`` if present, otherwise ``close``.
        window: lookback in trading days for both the momentum and the
            realised volatility components.

    Returns:
        Long-form factor frame. Per (symbol, timestamp):
            ``value = (close / close.shift(window) - 1) - rolling_std(pct_change, window)``.
        Earliest ``window`` rows per symbol are dropped (insufficient lookback).
    """

    if kline.is_empty():
        return _to_long(
            pl.DataFrame(schema={"symbol": pl.Utf8, "timestamp": pl.Datetime(time_zone="UTC"), "value": pl.Float64}),
