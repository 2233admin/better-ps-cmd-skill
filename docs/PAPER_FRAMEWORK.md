...
> 详见 `docs/research-references.md` — 竞品调研摘要

国内头部（幻方/明汯/启林/九坤）几乎全部黑箱——唯一可见的是幻方，因为它拆出 DeepSeek 必须开源 GPU 训练栈。量化策略本身**零公开**。

k-atana 的差异化优势：

| 维度 | 国内头部 | k-atana |
|------|---------|---------|
| PIT 数据契约 | 内部有，不公开 | **显式设计原则**，可证明无数据泄漏 |
| GPU batched HMM | 黑箱 | B=7504 × T=1889，6.4/8 状态激活，14-19x CPU 提速 |
| 慢熊检测 | 黑箱 | Multivariate HMM（XAR-465，待做）针对 2018/2021-22 补漏 |
| 多 agent 架构 | 黑箱 | 显式建模 + broadcast/negotiation 对比实验 |
| 因子研究可信度 | 黑箱 | PIT-correct 回测 + 与 naive 回测的 IC 差异对比（空白 2） |

**核心观察**：qlib 等开源工具抹平了因子研究层面的差距；执行基础设施（微秒级延迟）和另类数据是真实护城河，但不在本论文范围内。
