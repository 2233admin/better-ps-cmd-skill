...
| 5 | **指令具体性** | 15 | 不模糊、有具体参数/格式/示例、可直接执行<br><br>**评分参考**：1-3分=步骤含糊（如"处理X"无参数）；4-6分=有方向但缺示例；7-10分=每步有输入/输出格式、参数范围、错误处理 |
...
| 8 | **实测表现** | 25 | 用测试prompt跑一遍，输出质量是否符合skill宣称的能力<br><br>**评分参考**：1-5分=无测试体系；6-8分=dry_run；9-10分=真实子agent对比baseline<br><br>**实测验证**：2026-05-22 真实子agent测试（autoresearch skill），with-skill 66 vs baseline 58，完整Phase流程，+4分提升。**得分：9/10 → 23/25制** |
...
如果无法跑子agent，按以下规则精确触发 dry_run：

| 触发条件 | 处理方式 |
|---|---|
| Task/Agent 工具调用失败（超时、权限、工具不存在）| dry_run，note 记具体失败原因 |
| 子 agent 执行超过 120s 未返回 | dry_run，note 记「子agent超时」 |
| 环境不支持多 agent 并行 | dry_run，note 记「环境限制」 |
| 评估者判断时间/token 不够完成完整测试 | dry_run，note 记「资源不足」 |

dry_run 时须模拟完整执行路径并给出打分理由，results.tsv 标注 `dry_run`。
...
for each skill in 优化范围:
  ...
  # 汇总
  5. 计算加权总分（8维度rubric）
  6. 调用 skill_metrics.py 计算量化指标：
     ```bash
     PYTHONIOENCODING=utf-8 python .claude/skills/darwin-skill/scripts/skill_metrics.py        --results results.tsv --skill <name> --rubric-score <rubric_score>
     ```
     得到：综合评分 = rubric_score + 量化调整因子
  7. 记录到 results.tsv
```
...
```
┌──────────────────────────┬───────┬───────┬──────────────┬──────────────┐
│ Skill                    │ 8维   │ 综合 │ 结构短板      │ 效果短板      │
├──────────────────────────┼───────┼───────┼──────────────┼──────────────┤
│ huashu-proofreading      │ 78    │ 80    │ 边界条件      │ 测试prompt2  │
│ huashu-slides            │ 72    │ 73    │ 指令具体性    │ baseline持平  │
├──────────────────────────┼───────┼──────────────┼──────────────┤
│ 平均                     │ 75    │ 76    │              │
└──────────────────────────┴───────┴───────┴──────────────┴──────────────┘
```
...
> **注**：templates/ 和 scripts/ 位于上游 darwin-skill 仓库，不随本 SKILL.md 分发。使用时从上游仓库获取，或用 `npx skills add alchaincyf/darwin-skill --force` 拉取完整资源。
```
